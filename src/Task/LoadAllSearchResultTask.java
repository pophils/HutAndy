...
public class LoadAllSearchResultTask extends AsyncTask<String, Void, String> {     
	...
	public LoadAllSearchResultTask(Activity context, Intent searchIntent, AllSearchResult taskCaller){
    ...
    protected void onPostExecute(String jsonResult) {
		...
		try {
			...
			if(SearchAll.loadingGifDialog != null)
			   SearchAll.loadingGifDialog.dismiss();				
		 
			
			AllSearchResult.currentNumOfListingFetched = listingsJson.length();
			AllSearchResult.currentResultIndex = 1;	
			AllSearchResult.searchHeader = (TextView) ((SherlockFragmentActivity) context).findViewById(id.search_result_header_lbl);		
			taskCaller.updateSearchHeader(searchIntent.getBooleanExtra(Konstant.TAG_IS_FORSALE_SEARCH_XTRA, true));
			AllSearchResult.totalPgNo = listingJsonResult.getInt(Konstant.TAG_TOTAL_PAGE_NO);
